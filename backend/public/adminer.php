...
script("$bh.onclick = function () { return confirm('".($Me?js_escape($Me):'Are you sure?')."'); };","");}function
...
redirect($_,$Me=null){if($Me!==null){restart_session();$_SESSION["messages"][preg_replace('~^[^?]*~','',($_!==null?$_:$_SERVER["REQUEST_URI"]))][]=$Me;}if($_!==null){if($_=="")$_=".";header("Location: $_");exit;}}function
...
false;}if($yg)redirect($_,$Me.$xh);return
true;}function
...
upload_error($l){$Ie=($l==UPLOAD_ERR_INI_SIZE?ini_get("upload_max_filesize"):0);return($l?'Unable to upload a file.'.($Ie?" ".sprintf('Maximum allowed file size is %sB.',$Ie):""):'File does not exist.');}function
...
edit_form($P,$n,$I,$Hi){global$b,$w,$S,$l;$Nh=$b->tableName(table_status1($P,true));page_header(($Hi?'Edit':'Insert'),$l,array("select"=>array($P,$Nh)),$Nh);$b->editRowPrint($P,$n,$I,$Hi);if($I===false){echo"<p class='error'>".'No rows.'."\n";return;}echo'<form action="" method="post" enctype="multipart/form-data" id="form">
...
query($F,$_i=false){$G=$this->pdo->query($F);$this->error="";if(!$G){list(,$this->errno,$this->error)=$this->pdo->errorInfo();if(!$this->error)$this->error='Unknown error.';return
...
h($f->error);}function
check_sqlite_name($B){global$f;$Uc="db|sdb|sqlite";if(!preg_match("~^[^\\0]*\\.($Uc)\$~",$B)){$f->error=sprintf('Please use one of the extensions %s.',str_replace("|",", ",$Uc));return
false;}return
true;}function
create_database($j,$ob){global$f;if(file_exists($j)){$f->error='File exists.';return
...
Min_DB;$Nb=$b->credentials();if($f->connect($Nb[0],$Nb[1],$Nb[2])){if(min_version(9,0,$f)){$f->query("SET application_name = 'Adminer'");if(min_version(9.2,0,$f)){$Fh['Strings'][]="json";$U["json"]=4294967295;if(min_version(9.4,0,$f)){$Fh['Strings'][]="jsonb";$U["jsonb"]=4294967295;}}}return$f;}return$f->error;}function
get_databases(){return
...
set_schema($Wg,$g=null){global$f,$U,$Fh;if(!$g)$g=$f;$H=$g->query("SET search_path TO ".idf_escape($Wg));foreach(types()as$x=>$T){if(!isset($U[$T])){$U[$T]=$x;$Fh['User types'][]=$T;}}return$H;}function
...
create_sql($P,$Ka,$Gh){$Mg=array();$hh=array();$N=table_status($P);if(is_view($N)){$Wi=view($P);return
...
view($B){$Wi=views_table("view_name, text");$J=get_rows('SELECT text "select" FROM '.$Wi.' WHERE view_name = '.q($B));return
reset($J);}function
collations(){return
array();}function
information_schema($j){return
...
MongoClient($Ii,$C);$this->error='Database does not support password.';}catch(Exception$uc){}}}catch(Exception$uc){$this->error=$uc->getMessage();}}function
query($F){return
false;}function
...
selectLinks($Mh,$M=""){global$w,$k;echo'<p class="links">';$ye=array("select"=>'Select data');if(support("table")||support("indexes"))$ye["table"]='Show structure';if(support("table")){if(is_view($Mh))$ye["view"]='Alter view';else$ye["create"]='Alter table';}if($M!==null)$ye["edit"]='New item';$B=$Mh["Name"];foreach($ye
...
selectQuery($F,$Ch,$Wc=false){global$w,$k;$H="</p>\n";if(!$Wc&&($aj=$k->warnings())){$Jd="warnings";$H=", <a href='#$Jd'>".'Warnings'."</a>".script("qsl('a').onclick = partial(toggle, '$Jd');","")."$H<div id='$Jd' class='hidden'>\n$aj</div>\n";}return"<p><code class='jush-$w'>".h(str_replace("\n"," ",$F))."</code> <span class='time'>(".format_time($Ch).")</span>".(support("sql")?" <a href='".h(ME)."sql=".urlencode($F)."'>".'Edit'."</a>":"").$H;}function
...
selectVal($X,$z,$m,$Hf){$H=($X===null?"<i>NULL</i>":(preg_match("~char|binary|boolean~",$m["type"])&&!preg_match("~var~",$m["type"])?"<code>$X</code>":$X));if(preg_match('~blob|bytea|raw|file~',$m["type"])&&!is_utf8($X))$H="<i>".lang(array('%d byte','%d bytes'),strlen($Hf))."</i>";if(preg_match('~json~',$m["type"]))$H="<code class='jush-js'>$H</code>";return($z?"<a href='".h($z)."'".(is_url($z)?target_blank():"").">$H</a>":$H);}function
editVal($X,$m){return$X;}function
tableStructurePrint($n){global$Fh;echo"<div class='scrollable'>\n","<table class='nowrap odds'>\n","<thead><tr><th>".'Column'."<td>".'Type'.(support("comment")?"<td>".'Comment':"")."</thead>\n";foreach($n
as$m){echo"<tr><th>".h($m["field"]);$T=h($m["full_type"]);echo"<td><span title='".h($m["collation"])."'>".(in_array($T,(array)$Fh['User types'])?"<a href='".h(ME.'type='.urlencode($T))."'>$T</a>":$T)."</span>",($m["null"]?" <i>NULL</i>":""),($m["auto_increment"]?" <i>".'Auto Increment'."</i>":""),(isset($m["default"])?" <span title='".'Default value'."'>[<b>".h($m["default"])."</b>]</span>":""),(support("comment")?"<td>".h($m["comment"]):""),"\n";}echo"</table>\n","</div>\n";}function
...
selectColumnsPrint($K,$e){global$rd,$yd;print_fieldset("select",'Select',$K);$s=0;$K[""]=array();foreach($K
as$x=>$X){$X=$_GET["columns"][$x];$d=select_input(" name='columns[$s][col]'",$e,$X["col"],($x!==""?"selectFieldChange":"selectAddRow"));echo"<div>".($rd||$yd?"<select name='columns[$s][fun]'>".optionlist(array(-1=>"")+array_filter(array('Functions'=>$rd,'Aggregation'=>$yd)),$X["fun"])."</select>".on_help("getTarget(event).value && getTarget(event).value.replace(/ |\$/, '(') + ')'",1).script("qsl('select').onchange = function () { helpClose();".($x!==""?"":" qsl('select, input', this.parentNode).onchange();")." };","")."($d)":$d)."</div>\n";$s++;}echo"</div></fieldset>\n";}function
...
selectOrderPrint($_f,$e,$v){print_fieldset("sort",'Sort',$_f);$s=0;foreach((array)$_GET["order"]as$x=>$X){if($X!=""){echo"<div>".select_input(" name='order[$s]'",$e,$X,"selectFieldChange"),checkbox("desc[$s]",1,isset($_GET["desc"][$x]),'descending')."</div>\n";$s++;}}echo"<div>".select_input(" name='order[$s]'",$e,"","selectAddRow"),checkbox("desc[$s]",1,false,'descending')."</div>\n","</div></fieldset>\n";}function
...
messageQuery($F,$ci,$Wc=false){global$w,$k;restart_session();$Fd=&get_session("queries");if(!$Fd[$_GET["db"]])$Fd[$_GET["db"]]=array();if(strlen($F)>1e6)$F=preg_replace('~[\x80-\xFF]+$~','',substr($F,0,1e6))."\nâ€¦";$Fd[$_GET["db"]][]=array($F,time(),$ci);$zh="sql-".count($Fd[$_GET["db"]]);$H="<a href='#$zh' class='toggle'>".'SQL command'."</a>\n";if(!$Wc&&($aj=$k->warnings())){$Jd="warnings-".count($Fd[$_GET["db"]]);$H="<a href='#$Jd' class='toggle'>".'Warnings'."</a>, $H<div id='$Jd' class='hidden'>\n$aj</div>\n";}return" <span class='time'>".@date("H:i:s")."</span>"." $H<div id='$zh' class='hidden'><pre><code class='jush-$w'>".shorten_utf8($F,1000)."</code></pre>".($ci?" <span class='time'>($ci)</span>":'').(support("sql")?'<p><a href="'.h(str_replace("db=".urlencode(DB),"db=".urlencode($_GET["db"]),ME).'sql=&history='.(count($Fd[$_GET["db"]])-1)).'">'.'Edit'.'</a>':'').'</div>';}function
...
as$Yf=>$X){if(!$Yf||preg_match("~$Yf~",$m["type"]))$H.="/$X";}}if($x&&!preg_match('~set|blob|bytea|raw|file|bool~',$m["type"]))$H.="/SQL";}if($m["auto_increment"]&&!$Hi)$H='Auto Increment';return
explode("/",$H);}function
editInput($P,$m,$Ha,$Y){if($m["type"]=="enum")return(isset($_GET["select"])?"<label><input type='radio'$Ha value='-1' checked><i>".'original'."</i></label> ":"").($m["null"]?"<label><input type='radio'$Ha value=''".($Y!==null||isset($_GET["select"])?"":" checked")."><i>NULL</i></label> ":"").enum_input("radio",$Ha,$m,$Y,$Y===0?0:null);return"";}function
...
connect($L,$V,$E){if(ini_bool("mysql.allow_local_infile")){$this->error=sprintf('Disable %s or enable %s or %s extensions.',"'mysql.allow_local_infile'","MySQLi","PDO_MySQL");return
...
Min_DB;$Nb=$b->credentials();if($f->connect($Nb[0],$Nb[1],$Nb[2])){$f->set_charset(charset($f));$f->query("SET sql_quote_show_create = 1, autocommit = 1");if(min_version('5.7.8',10.2,$f)){$Fh['Strings'][]="json";$U["json"]=4294967295;}if(min_version('',10.7,$f)){$Fh['Strings'][]="uuid";$U["uuid"]=128;$vc[0]['uuid']='uuid';}if(min_version(9,'',$f)){$Fh['Numbers'][]="vector";$U["vector"]=16383;$vc[0]['vector']='string_to_vector';}return$f;}$H=$f->error;if(function_exists('iconv')&&!is_utf8($H)&&strlen($Ug=iconv("windows-1250","utf-8",$H))>strlen($H))$H=$Ug;return$H;}function
...
as$P)$dc[table($P)]=view($P);$f->select_db($Vh);$j=idf_escape(DB);foreach($dc
...
false;}}foreach($Xi
as$P){$B=($Vh==DB?table("copy_$P"):idf_escape($Vh).".".table($P));$Wi=view($P);if(($_POST["overwrite"]&&!queries("DROP VIEW IF EXISTS $B"))||!queries("CREATE VIEW $B AS $Wi[select]"))return
false;}return
true;}function
trigger($B){if($B=="")return
...
check_invalid_login(){global$b;$ae=unserialize(@file_get_contents(get_temp_dir()."/adminer.invalid"));$Zd=($ae?$ae[$b->bruteForceKey()]:array());$cf=($Zd[1]>29?$Zd[0]-time():0);if($cf>0)auth_error(lang(array('Too many unsuccessful logins, try again in %d minute.','Too many unsuccessful logins, try again in %d minutes.'),ceil($cf/60)));}$Ia=$_POST["auth"];if($Ia){session_regenerate_id();$Ui=$Ia["driver"];$L=$Ia["server"];$V=$Ia["username"];$E=(string)$Ia["password"];$j=$Ia["db"];set_password($Ui,$L,$V,$E);$_SESSION["db"][$Ui][$L][$V][$j]=true;if($Ia["permanent"]){$x=base64_encode($Ui)."-".base64_encode($L)."-".base64_encode($V)."-".base64_encode($j);$mg=$b->permanentLogin(true);$ag[$x]="$x:".base64_encode($mg?encrypt_string($E,$mg):"");cookie("adminer_permanent",implode(" ",$ag));}if(count($_POST)==1||DRIVER!=$Ui||SERVER!=$L||$_GET["username"]!==$V||DB!=$j)redirect(auth_url($Ui,$L,$V,$j));}elseif($_POST["logout"]&&(!$Cd||verify_token())){foreach(array("pwds","db","dbs","queries")as$x)set_session($x,null);unset_permanent();redirect(substr(preg_replace('~\b(username|db|ns)=[^&]*&~','',ME),0,-1),'Logout successful.'.' '.'Thanks for using Adminer, consider <a href="https://www.adminer.org/en/donation/">donating</a>.');}elseif($ag&&!$_SESSION["pwds"]){session_regenerate_id();$mg=$b->permanentLogin();foreach($ag
...
as$x=>$X){list($Ui,$L,$V,$j)=array_map('base64_decode',explode("-",$x));if($Ui==DRIVER&&$L==SERVER&&$V==$_GET["username"]&&$j==DB)unset($ag[$x]);}cookie("adminer_permanent",implode(" ",$ag));}function
auth_error($l){global$b,$Cd;$kh=session_name();if(isset($_GET["username"])){header("HTTP/1.1 403 Forbidden");if(($_COOKIE[$kh]||$_GET[$kh])&&!$Cd)$l='Session expired, please login again.';else{restart_session();add_invalid_login();$E=get_password();if($E!==null){if($E===false)$l.=($l?'<br>':'').sprintf('Master password expired. <a href="https://www.adminer.org/en/extension/"%s>Implement</a> %s method to make it permanent.',target_blank(),'<code>permanentLogin()</code>');set_password(DRIVER,SERVER,$_GET["username"],null);}unset_permanent();}}if(!$_COOKIE[$kh]&&$_GET[$kh]&&ini_bool("session.use_only_cookies"))$l='Session support must be enabled.';$Pf=session_get_cookie_params();cookie("adminer_key",($_COOKIE["adminer_key"]?$_COOKIE["adminer_key"]:rand_string()),$Pf["lifetime"]);page_header('Login',$l,null);echo"<form action='' method='post'>\n","<div>";if(hidden_fields($_POST,array("auth")))echo"<p class='message'>".'The action will be performed after successful login with the same credentials.'."\n";echo"</div>\n";$b->loginForm();echo"</form>\n";page_footer("auth");exit;}if(isset($_GET["username"])&&!class_exists("Min_DB")){unset($_SESSION["pwds"][DRIVER]);unset_permanent();page_header('No extension',sprintf('None of the supported PHP extensions (%s) are available.',implode(", ",$gg)),false);page_footer("auth");exit;}stop_session(true);if(isset($_GET["username"])&&is_string(get_password())){list($Hd,$cg)=explode(":",SERVER,2);if(preg_match('~^\s*([-+]?\d+)~',$cg,$A)&&($A[1]<1024||$A[1]>65535))auth_error('Connecting to privileged ports is not allowed.');check_invalid_login();$f=connect();$k=new
Min_Driver($f);}$ze=null;if(!is_object($f)||($ze=$b->login($_GET["username"],get_password()))!==true){$l=(is_string($f)?nl_br(h($f)):(is_string($ze)?$ze:'Invalid credentials.'));auth_error($l.(preg_match('~^ | $~',get_password())?'<br>'.'There is a space in the input password which might be the cause.':''));}if($_POST["logout"]&&$Cd&&!verify_token()){page_header('Logout','Invalid CSRF token. Send the form again.');page_footer("db");exit;}if($Ia&&$_POST["token"])$_POST["token"]=$S;$l='';if($_POST){if(!verify_token()){$Ud="max_input_vars";$Je=ini_get($Ud);if(extension_loaded("suhosin")){foreach(array("suhosin.request.max_vars","suhosin.post.max_vars")as$x){$X=ini_get($x);if($X&&(!$Je||$X<$Je)){$Ud=$x;$Je=$X;}}}$l=(!$_POST["token"]&&$Je?sprintf('Maximum number of allowed fields exceeded. Please increase %s.',"'$Ud'"):'Invalid CSRF token. Send the form again.'.' '.'If you did not send this request from Adminer then close this page.');}}elseif($_SERVER["REQUEST_METHOD"]=="POST"){$l=sprintf('Too big POST data. Reduce the data or increase the %s configuration directive.',"'post_max_size'");if(isset($_GET["sql"]))$l.=' '.'You can upload a big SQL file via FTP and import it from server.';}function
...
<td><input type="radio" name="auto_increment_col" value=""><abbr id="label-ai" title="Auto Increment">AI</abbr>',doc_link(array('sql'=>"example-auto-increment.html",'mariadb'=>"auto_increment/",'sqlite'=>"autoinc.html",'pgsql'=>"datatype-numeric.html#DATATYPE-SERIAL",'mssql'=>"t-sql/statements/create-table-transact-sql-identity-property",)),'<td id="label-default"',$bc,'>Default value
',(support("comment")?"<td id='label-comment'$wb>".'Comment':"");}echo'<td>',"<input type='image' class='icon' name='add[".(support("move_col")?0:count($n))."]' src='".h(preg_replace("~\\?.*~","",ME)."?file=plus.gif&version=4.17.1")."' alt='+' title='".'Add next'."'>".script("row_count = ".count($n).";"),'</thead>
<tbody>
...
',($T=="PROCEDURE"?"<td>".html_select("fields[$s][inout]",explode("|",$Vd),$m["inout"]):""),'<th>';if($kc){echo'<input name="fields[',$s,'][field]" value="',h($m["field"]),'" data-maxlength="64" autocapitalize="off" aria-labelledby="label-name">';}echo'<input type="hidden" name="fields[',$s,'][orig]" value="',h($Df),'">';edit_type("fields[$s]",$m,$pb,$md);if($T=="TABLE"){echo'<td>',checkbox("fields[$s][null]",1,$m["null"],"","","block","label-null"),'<td><label class="block"><input type="radio" name="auto_increment_col" value="',$s,'"';if($m["auto_increment"]){echo' checked';}echo' aria-labelledby="label-ai"></label><td',$bc,'>',checkbox("fields[$s][has_default]",1,$m["has_default"],"","","","label-default"),'<input name="fields[',$s,'][default]" value="',h($m["default"]),'" aria-labelledby="label-default">',(support("comment")?"<td$wb><input name='fields[$s][comment]' value='".h($m["comment"])."' data-maxlength='".(min_version(5.5)?1024:255)."' aria-labelledby='label-comment'>":"");}echo"<td>",(support("move_col")?"<input type='image' class='icon' name='add[$s]' src='".h(preg_replace("~\\?.*~","",ME)."?file=plus.gif&version=4.17.1")."' alt='+' title='".'Add next'."'> "."<input type='image' class='icon' name='up[$s]' src='".h(preg_replace("~\\?.*~","",ME)."?file=up.gif&version=4.17.1")."' alt='â†‘' title='".'Move up'."'> "."<input type='image' class='icon' name='down[$s]' src='".h(preg_replace("~\\?.*~","",ME)."?file=down.gif&version=4.17.1")."' alt='â†“' title='".'Move down'."'> ":""),($Df==""||support("drop_col")?"<input type='image' class='icon' name='drop_col[$s]' src='".h(preg_replace("~\\?.*~","",ME)."?file=cross.gif&version=4.17.1")."' alt='x' title='".'Remove'."'>":"");}}function
...
connect_error(){global$b,$f,$S,$l,$nc;if(DB!=""){header("HTTP/1.1 404 Not Found");page_header('Database'.": ".h(DB),'Invalid database.',true);}else{if($_POST["db"]&&!$l)queries_redirect(substr(ME,0,-1),'Databases have been dropped.',drop_databases($_POST["db"]));page_header('Select database',$l,false);echo"<p class='links'>\n";foreach(array('database'=>'Create database','privileges'=>'Privileges','processlist'=>'Process list','variables'=>'Variables','status'=>'Status',)as$x=>$X){if(support($x))echo"<a href='".h(ME)."$x='>$X</a>\n";}echo"<p>".sprintf('%s version: %s through PHP extension %s',$nc[DRIVER],"<b>".h($f->server_info)."</b>","<b>$f->extension</b>")."\n","<p>".sprintf('Logged as: %s',"<b>".h(logged_user())."</b>")."\n";$i=$b->databases();if($i){$Xg=support("scheme");$pb=collations();echo"<form action='' method='post'>\n","<table class='checkable odds'>\n",script("mixin(qsl('table'), {onclick: tableClick, ondblclick: partialArg(tableClick, true)});"),"<thead><tr>".(support("database")?"<td>":"")."<th>".'Database'." - <a href='".h(ME)."refresh=1'>".'Refresh'."</a>"."<td>".'Collation'."<td>".'Tables'."<td>".'Size'." - <a href='".h(ME)."dbsize=1'>".'Compute'."</a>".script("qsl('a').onclick = partial(ajaxSetHtml, '".js_escape(ME)."script=connect');","")."</thead>\n";$i=($_GET["dbsize"]?count_tables($i):array_flip($i));foreach($i
as$j=>$R){$Pg=h(ME)."db=".urlencode($j);$Jd=h("Db-".$j);echo"<tr>".(support("database")?"<td>".checkbox("db[]",$j,in_array($j,(array)$_POST["db"]),"","","",$Jd):""),"<th><a href='$Pg' id='$Jd'>".h($j)."</a>";$ob=h(db_collation($j,$pb));echo"<td>".(support("database")?"<a href='$Pg".($Xg?"&amp;ns=":"")."&amp;database=' title='".'Alter database'."'>$ob</a>":$ob),"<td align='right'><a href='$Pg&amp;schema=' id='tables-".h($j)."' title='".'Database schema'."'>".($_GET["dbsize"]?$R:"?")."</a>","<td align='right' id='size-".h($j)."'>".($_GET["dbsize"]?db_size($j):"?"),"\n";}echo"</table>\n",(support("database")?"<div class='footer'><div>\n"."<fieldset><legend>".'Selected'." <span id='selected'></span></legend><div>\n"."<input type='hidden' name='all' value=''>".script("qsl('input').onclick = function () { selectCount('selected', formChecked(this, /^db/)); };")."<input type='submit' name='drop' value='".'Drop'."'>".confirm()."\n"."</div></fieldset>\n"."</div></div>\n":""),"<input type='hidden' name='token' value='$S'>\n","</form>\n",script("tableCheck();");}}page_footer("db");}if(isset($_GET["status"]))$_GET["variables"]=$_GET["status"];if(isset($_GET["import"]))$_GET["sql"]=$_GET["import"];if(!(DB!=""?$f->select_db(DB):isset($_GET["sql"])||isset($_GET["dump"])||isset($_GET["database"])||isset($_GET["processlist"])||isset($_GET["privileges"])||isset($_GET["user"])||isset($_GET["variables"])||$_GET["script"]=="connect"||$_GET["script"]=="kill")){if(DB!=""||$_GET["refresh"]){restart_session();set_session("dbs",null);}connect_error();exit;}if(support("scheme")){if(DB!=""&&$_GET["ns"]!==""){if(!isset($_GET["ns"]))redirect(preg_replace('~ns=[^&]*&~','',ME)."ns=".get_schema());if(!set_schema($_GET["ns"])){header("HTTP/1.1 404 Not Found");page_header('Schema'.": ".h($_GET["ns"]),'Invalid schema.',true);page_footer("ns");exit;}}}$rf="RESTRICT|NO ACTION|CASCADE|SET NULL|SET DEFAULT";class
TmpFile{var$handler;var$size;function
__construct(){$this->handler=tmpfile();}function
write($Fb){$this->size+=strlen($Fb);fwrite($this->handler,$Fb);}function
send(){fseek($this->handler,0);fpassthru($this->handler);fclose($this->handler);}}$Ec="'(?:''|[^'\\\\]|\\\\.)*'";$Vd="IN|OUT|INOUT";if(isset($_GET["select"])&&($_POST["edit"]||$_POST["clone"])&&!$_POST["save"])$_GET["edit"]=$_GET["select"];if(isset($_GET["callf"]))$_GET["call"]=$_GET["callf"];if(isset($_GET["function"]))$_GET["procedure"]=$_GET["function"];if(isset($_GET["download"])){$a=$_GET["download"];$n=fields($a);header("Content-Type: application/octet-stream");header("Content-Disposition: attachment; filename=".friendly_url("$a-".implode("_",$_GET["where"])).".".friendly_url($_GET["field"]));$K=array(idf_escape($_GET["field"]));$G=$k->select($a,$K,array(where($_GET,$n)),$K);$I=($G?$G->fetch_row():array());echo$k->value($I[0],$n[$_GET["field"]]);exit;}elseif(isset($_GET["table"])){$a=$_GET["table"];$n=fields($a);if(!$n)$l=error();$Q=table_status1($a,true);$B=$b->tableName($Q);page_header(($n&&is_view($Q)?$Q['Engine']=='materialized view'?'Materialized view':'View':'Table').": ".($B!=""?$B:h($a)),$l);$Og=array();foreach($n
as$x=>$m)$Og+=$m["privileges"];$b->selectLinks($Q,(isset($Og["insert"])||!support("table")?"":null));$vb=$Q["Comment"];if($vb!="")echo"<p class='nowrap'>".'Comment'.": ".h($vb)."\n";if($n)$b->tableStructurePrint($n);if(!is_view($Q)){if(support("indexes")){echo"<h3 id='indexes'>".'Indexes'."</h3>\n";$v=indexes($a);if($v)$b->tableIndexesPrint($v);echo'<p class="links"><a href="'.h(ME).'indexes='.urlencode($a).'">'.'Alter indexes'."</a>\n";}if(fk_support($Q)){echo"<h3 id='foreign-keys'>".'Foreign keys'."</h3>\n";$md=foreign_keys($a);if($md){echo"<table>\n","<thead><tr><th>".'Source'."<td>".'Target'."<td>".'ON DELETE'."<td>".'ON UPDATE'."<td></thead>\n";foreach($md
...
as$x=>$X){echo"<tr title='".h($x)."'>","<td><code class='jush-$w'>".h($X),"<td><a href='".h(ME.'check='.urlencode($a).'&name='.urlencode($x))."'>".'Alter'."</a>","\n";}echo"</table>\n";}echo'<p class="links"><a href="'.h(ME).'check='.urlencode($a).'">'.'Create check'."</a>\n";}}if(support(is_view($Q)?"view_trigger":"trigger")){echo"<h3 id='triggers'>".'Triggers'."</h3>\n";$xi=triggers($a);if($xi){echo"<table>\n";foreach($xi
as$x=>$X)echo"<tr valign='top'><td>".h($X[0])."<td>".h($X[1])."<th>".h($x)."<td><a href='".h(ME.'trigger='.urlencode($a).'&name='.urlencode($x))."'>".'Alter'."</a>\n";echo"</table>\n";}echo'<p class="links"><a href="'.h(ME).'trigger='.urlencode($a).'">'.'Add trigger'."</a>\n";}}elseif(isset($_GET["schema"])){page_header('Database schema',"",array(),h(DB.($_GET["ns"]?".$_GET[ns]":"")));$Ph=array();$Qh=array();$ea=($_GET["schema"]?$_GET["schema"]:$_COOKIE["adminer_schema-".str_replace(".","_",DB)]);preg_match_all('~([^:]+):([-0-9.]+)x([-0-9.]+)(_|$)~',$ea,$De,PREG_SET_ORDER);foreach($De
...
';}elseif(isset($_GET["dump"])){$a=$_GET["dump"];if($_POST&&!$l){$Ib="";foreach(array("output","format","db_style","routines","events","table_style","auto_increment","triggers","data_style")as$x)$Ib.="&$x=".urlencode($_POST[$x]);cookie("adminer_export",substr($Ib,1));$R=array_flip((array)$_POST["tables"])+array_flip((array)$_POST["data"]);$Sc=dump_headers((count($R)==1?key($R):DB),(DB==""||count($R)>1));$de=preg_match('~sql~',$_POST["format"]);if($de){echo"-- Adminer $ia ".$nc[DRIVER]." ".str_replace("\n"," ",$f->server_info)." dump\n\n";if($w=="sql"){echo"SET NAMES utf8;
SET time_zone = '+00:00';
...
';$dd=true;foreach($ig
as$x=>$X){if($x!=""&&$X>1){echo($dd?"<p>":" ")."<a href='".h(ME)."dump=".urlencode("$x%")."'>".h($x)."</a>";$dd=false;}}}elseif(isset($_GET["privileges"])){page_header('Privileges');echo'<p class="links"><a href="'.h(ME).'user=">'.'Create user'."</a>";$G=$f->query("SELECT User, Host FROM mysql.".(DB==""?"user":"db WHERE ".q(DB)." LIKE Db")." ORDER BY Host, User");$td=$G;if(!$G)$G=$f->query("SELECT SUBSTRING_INDEX(CURRENT_USER, '@', 1) AS User, SUBSTRING_INDEX(CURRENT_USER, '@', -1) AS Host");echo"<form action=''><p>\n";hidden_fields_get();echo"<input type='hidden' name='db' value='".h(DB)."'>\n",($td?"":"<input type='hidden' name='grant' value=''>\n"),"<table class='odds'>\n","<thead><tr><th>".'Username'."<th>".'Server'."<th></thead>\n";while($I=$G->fetch_assoc())echo'<tr><td>'.h($I["User"])."<td>".h($I["Host"]).'<td><a href="'.h(ME.'user='.urlencode($I["User"]).'&host='.urlencode($I["Host"])).'">'.'Edit'."</a>\n";if(!$td||DB!="")echo"<tr><td><input name='user' autocapitalize='off'><td><input name='host' value='localhost' autocapitalize='off'><td><input type='submit' value='".'Edit'."'>\n";echo"</table>\n","</form>\n";}elseif(isset($_GET["sql"])){if(!$l&&$_POST["export"]){dump_headers("sql");$b->dumpTable("","");$b->dumpData("","table",$_POST["query"]);exit;}restart_session();$Gd=&get_session("queries");$Fd=&$Gd[DB];if(!$l&&$_POST["clear"]){$Fd=array();redirect(remove_from_uri("history"));}page_header((isset($_GET["import"])?'Import':'SQL command'),$l);if(!$l&&$_POST){$q=false;if(!isset($_GET["import"]))$F=$_POST["query"];elseif($_POST["webfile"]){$yh=$b->importServerPath();$q=@fopen((file_exists($yh)?$yh:"compress.zlib://$yh.gz"),"rb");$F=($q?fread($q,1e6):false);}else$F=get_file("sql_file",true);if(is_string($F)){if(function_exists('memory_get_usage')&&($Le=ini_bytes("memory_limit"))!="-1")@ini_set("memory_limit",max($Le,2*strlen($F)+memory_get_usage()+8e6));if($F!=""&&strlen($F)<1e6){$sg=$F.(preg_match("~;[ \t\r\n]*\$~",$F)?"":";");if(!$Fd||reset(end($Fd))!=$sg){restart_session();$Fd[]=array($sg,time());set_session("queries",$Gd);stop_session();}}$vh="(?:\\s|/\\*[\s\S]*?\\*/|(?:#|-- )[^\n]*\n?|--\r?\n)";$ec=";";$jf=0;$Bc=true;$g=connect();if(is_object($g)&&DB!=""){$g->select_db(DB);if($_GET["ns"]!="")set_schema($_GET["ns"],$g);}$ub=0;$Hc=array();$Qf='[\'"'.($w=="sql"?'`#':($w=="sqlite"?'`[':($w=="mssql"?'[':''))).']|/\*|-- |$'.($w=="pgsql"?'|\$[^$]*\$':'');$ni=microtime(true);parse_str($_COOKIE["adminer_export"],$xa);$tc=$b->dumpFormat();unset($tc["sql"]);while($F!=""){if(!$jf&&preg_match("~^$vh*+DELIMITER\\s+(\\S+)~i",$F,$A)){$ec=$A[1];$F=substr($F,strlen($A[0]));}else{preg_match('('.preg_quote($ec)."\\s*|$Qf)",$F,$A,PREG_OFFSET_CAPTURE,$jf);list($od,$dg)=$A[0];if(!$od&&$q&&!feof($q))$F.=fread($q,1e5);else{if(!$od&&rtrim($F)=="")break;$jf=$dg+strlen($od);if($od&&rtrim($od)!=$ec){$Ya=$k->hasCStyleEscapes()||($w=="pgsql"&&($dg>0&&strtolower($F[$dg-1])=="e"));$Yf=($od=='/*'?'\*/':($od=='['?']':(preg_match('~^-- |^#~',$od)?"\n":preg_quote($od).($Ya?"|\\\\.":""))));while(preg_match("($Yf|\$)s",$F,$A,PREG_OFFSET_CAPTURE,$jf)){$Ug=$A[0][0];if(!$Ug&&$q&&!feof($q))$F.=fread($q,1e5);else{$jf=$A[0][1]+strlen($Ug);if(!$Ug||$Ug[0]!="\\")break;}}}else{$Bc=false;$sg=substr($F,0,$dg);$ub++;$lg="<pre id='sql-$ub'><code class='jush-$w'>".$b->sqlCommandQuery($sg)."</code></pre>\n";if($w=="sqlite"&&preg_match("~^$vh*+ATTACH\\b~i",$sg,$A)){echo$lg,"<p class='error'>".'ATTACH queries are not supported.'."\n";$Hc[]=" <a href='#sql-$ub'>$ub</a>";if($_POST["error_stops"])break;}else{if(!$_POST["only_errors"]){echo$lg;ob_flush();flush();}$Ch=microtime(true);if($f->multi_query($sg)&&is_object($g)&&preg_match("~^$vh*+USE\\b~i",$sg))$g->query($sg);do{$G=$f->store_result();if($f->error){echo($_POST["only_errors"]?$lg:""),"<p class='error'>".'Error in query'.($f->errno?" ($f->errno)":"").": ".error()."\n";$Hc[]=" <a href='#sql-$ub'>$ub</a>";if($_POST["error_stops"])break
2;}else{$ci=" <span class='time'>(".format_time($Ch).")</span>".(strlen($sg)<1000?" <a href='".h(ME)."sql=".urlencode(trim($sg))."'>".'Edit'."</a>":"");$za=$f->affected_rows;$aj=($_POST["only_errors"]?"":$k->warnings());$bj="warnings-$ub";if($aj)$ci.=", <a href='#$bj'>".'Warnings'."</a>".script("qsl('a').onclick = partial(toggle, '$bj');","");$Pc=null;$Qc="explain-$ub";if(is_object($G)){$y=$_POST["limit"];$Cf=select($G,$g,array(),$y);if(!$_POST["only_errors"]){echo"<form action='' method='post'>\n";$ff=$G->num_rows;echo"<p>".($ff?($y&&$ff>$y?sprintf('%d / ',$y):"").lang(array('%d row','%d rows'),$ff):""),$ci;if($g&&preg_match("~^($vh|\\()*+SELECT\\b~i",$sg)&&($Pc=explain($g,$sg)))echo", <a href='#$Qc'>Explain</a>".script("qsl('a').onclick = partial(toggle, '$Qc');","");$Jd="export-$ub";echo", <a href='#$Jd'>".'Export'."</a>".script("qsl('a').onclick = partial(toggle, '$Jd');","")."<span id='$Jd' class='hidden'>: ".html_select("output",$b->dumpOutput(),$xa["output"])." ".html_select("format",$tc,$xa["format"])."<input type='hidden' name='query' value='".h($sg)."'>"." <input type='submit' name='export' value='".'Export'."'><input type='hidden' name='token' value='$S'></span>\n"."</form>\n";}}else{if(preg_match("~^$vh*+(CREATE|DROP|ALTER)$vh++(DATABASE|SCHEMA)\\b~i",$sg)){restart_session();set_session("dbs",null);stop_session();}if(!$_POST["only_errors"])echo"<p class='message' title='".h($f->info)."'>".lang(array('Query executed OK, %d row affected.','Query executed OK, %d rows affected.'),$za)."$ci\n";}echo($aj?"<div id='$bj' class='hidden'>\n$aj</div>\n":"");if($Pc){echo"<div id='$Qc' class='hidden explain'>\n";select($Pc,$g,$Cf);echo"</div>\n";}}$Ch=microtime(true);}while($f->next_result());}$F=substr($F,$jf);$jf=0;}}}}if($Bc)echo"<p class='message'>".'No commands to execute.'."\n";elseif($_POST["only_errors"]){echo"<p class='message'>".lang(array('%d query executed OK.','%d queries executed OK.'),$ub-count($Hc))," <span class='time'>(".format_time($ni).")</span>\n";}elseif($Hc&&$ub>1)echo"<p class='error'>".'Error in query'.": ".implode("",$Hc)."\n";}else
echo"<p class='error'>".upload_error($F)."\n";}echo'
<form action="" method="post" enctype="multipart/form-data" id="form">
';$Nc="<input type='submit' value='".'Execute'."' title='Ctrl+Enter'>";if(!isset($_GET["import"])){$sg=$_GET["sql"];if($_POST)$sg=$_POST["query"];elseif($_GET["history"]=="all")$sg=$Fd;elseif($_GET["history"]!="")$sg=$Fd[$_GET["history"]][0];echo"<p>";textarea("query",$sg,20);echo
script(($_POST?"":"qs('textarea').focus();\n")."qs('#form').onsubmit = partial(sqlSubmit, qs('#form'), '".js_escape(remove_from_uri("sql|limit|error_stops|only_errors|history"))."');"),"<p>$Nc\n",'Limit rows'.": <input type='number' name='limit' class='size' value='".h($_POST?$_POST["limit"]:$_GET["limit"])."'>\n";}else{echo"<fieldset><legend>".'File upload'."</legend><div>";$zd=(extension_loaded("zlib")?"[.gz]":"");echo(ini_bool("file_uploads")?"SQL$zd (&lt; ".ini_get("upload_max_filesize")."B): <input type='file' name='sql_file[]' multiple>\n$Nc":'File uploads are disabled.'),"</div></fieldset>\n";$Md=$b->importServerPath();if($Md){echo"<fieldset><legend>".'From server'."</legend><div>",sprintf('Webserver file %s',"<code>".h($Md)."$zd</code>"),' <input type="submit" name="webfile" value="'.'Run file'.'">',"</div></fieldset>\n";}echo"<p>";}echo
checkbox("error_stops",1,($_POST?$_POST["error_stops"]:isset($_GET["import"])||$_GET["error_stops"]),'Stop on error')."\n",checkbox("only_errors",1,($_POST?$_POST["only_errors"]:isset($_GET["import"])||$_GET["only_errors"]),'Show only errors')."\n","<input type='hidden' name='token' value='$S'>\n";if(!isset($_GET["import"])&&$Fd){print_fieldset("history",'History',$_GET["history"]!="");for($X=end($Fd);$X;$X=prev($Fd)){$x=key($Fd);list($sg,$ci,$xc)=$X;echo'<a href="'.h(ME."sql=&history=$x").'">'.'Edit'."</a>"." <span class='time' title='".@date('Y-m-d',$ci)."'>".@date("H:i:s",$ci)."</span>"." <code class='jush-$w'>".shorten_utf8(ltrim(str_replace("\n"," ",str_replace("\r","",preg_replace('~^(#|-- ).*~m','',$sg)))),80,"</code>").($xc?" <span class='time'>($xc)</span>":"")."<br>\n";}echo"<input type='submit' name='clear' value='".'Clear'."'>\n","<a href='".h(ME."sql=&history=all")."'>".'Edit all'."</a>\n","</div></fieldset>\n";}echo'</form>
...
as$B=>$m){if(!isset($m["privileges"][$Hi?"update":"insert"])||$b->fieldName($m)==""||$m["generated"])unset($n[$B]);}if($_POST&&!$l&&!isset($_GET["select"])){$_=$_POST["referer"];if($_POST["insert"])$_=($Hi?null:$_SERVER["REQUEST_URI"]);elseif(!preg_match('~^.+&select=.+$~',$_))$_=ME."select=".urlencode($a);$v=indexes($a);$Ci=unique_array($_GET["where"],$v);$vg="\nWHERE $Z";if(isset($_POST["delete"]))queries_redirect($_,'Item has been deleted.',$k->delete($a,$vg,!$Ci));else{$M=array();foreach($n
as$B=>$m){$X=process_input($m);if($X!==false&&$X!==null)$M[idf_escape($B)]=$X;}if($Hi){if(!$M)redirect($_);queries_redirect($_,'Item has been updated.',$k->update($a,$M,$vg,!$Ci));if(is_ajax()){page_headers();page_messages($l);exit;}}else{$G=$k->insert($a,$M);$qe=($G?last_id():0);queries_redirect($_,sprintf('Item%s has been inserted.',($qe?" $qe":"")),$G);}}}$I=null;if($_POST["save"])$I=(array)$_POST["fields"];elseif($Z){$K=array();foreach($n
...
',($Dc?"<select name='Engine'>".optionlist(array(""=>"(".'engine'.")")+$Dc,$I["Engine"])."</select>".on_help("getTarget(event).value",1).script("qsl('select').onchange = helpClose;"):""),' ',($pb&&!preg_match("~sqlite|mssql~",$w)?html_select("Collation",array(""=>"(".'collation'.")")+$pb,$I["Collation"]):""),' <input type="submit" value="Save">
';}echo'
...
',checkbox("defaults",1,($_POST?$_POST["defaults"]:adminer_setting("defaults")),'Default values',"columnShow(this.checked, 5)","jsonly");$xb=($_POST?$_POST["comments"]:adminer_setting("comments"));echo(support("comment")?checkbox("comments",1,$xb,'Comment',"editingCommentsClick(this, true);","jsonly").' '.(preg_match('~\n~',$I["Comment"])?"<textarea name='Comment' rows='2' cols='20'".($xb?"":" class='hidden'").">".h($I["Comment"])."</textarea>":'<input name="Comment" value="'.h($I["Comment"]).'" data-maxlength="'.(min_version(5.5)?2048:60).'"'.($xb?"":" class='hidden'").'>'):''),'<p>
...
<th><input type="submit" class="wayoff">','Column'.($we?"<span class='idxopts".($nh?"":" hidden")."'> (".'length'.")</span>":"");if($we||support("descidx"))echo
checkbox("options",1,$nh,'Options',"indexOptionsShow(this.checked)","jsonly")."\n";echo'<th id="label-name">Name
<th><noscript>',"<input type='image' class='icon' name='add[0]' src='".h(preg_replace("~\\?.*~","",ME)."?file=plus.gif&version=4.17.1")."' alt='+' title='".'Add next'."'>",'</noscript>
</thead>
';if($jg){echo"<tr><td>PRIMARY<td>";foreach($jg["columns"]as$x=>$d){echo
select_input(" disabled",$n,$d),"<label><input disabled type='checkbox'>".'descending'."</label> ";}echo"<td><td>\n";}$ge=1;foreach($I["indexes"]as$u){if(!$_POST["drop_col"]||$ge!=key($_POST["drop_col"])){echo"<tr><td>".html_select("indexes[$ge][type]",array(-1=>"")+$Qd,$u["type"],($ge==count($I["indexes"])?"indexesAddRow.call(this);":1),"label-type"),"<td>";ksort($u["columns"]);$s=1;foreach($u["columns"]as$x=>$d){echo"<span>".select_input(" name='indexes[$ge][columns][$s]' title='".'Column'."'",($n?array_combine($n,$n):$n),$d,"partial(".($s==count($u["columns"])?"indexesAddColumn":"indexesChangeColumn").", '".js_escape($w=="sql"?"":$_GET["indexes"]."_")."')"),"<span class='idxopts".($nh?"":" hidden")."'>",($we?"<input type='number' name='indexes[$ge][lengths][$s]' class='size' value='".h($u["lengths"][$x])."' title='".'Length'."'>":""),(support("descidx")?checkbox("indexes[$ge][descs][$s]",1,$u["descs"][$x],'descending'):""),"</span> </span>";$s++;}echo"<td><input name='indexes[$ge][name]' value='".h($u["name"])."' autocapitalize='off' aria-labelledby='label-name'>\n","<td><input type='image' class='icon' name='drop_col[$ge]' src='".h(preg_replace("~\\?.*~","",ME)."?file=cross.gif&version=4.17.1")."' alt='x' title='".'Remove'."'>".script("qsl('input').onclick = partial(editingRemoveRow, 'indexes\$1[type]');");}$ge++;}echo'</table>
</div>
<p>
<input type="submit" value="Save">
<input type="hidden" name="token" value="',$S,'">
</form>
';}elseif(isset($_GET["database"])){$I=$_POST;if($_POST&&!$l&&!isset($_POST["add_x"])){$B=trim($I["name"]);if($_POST["drop"]){$_GET["db"]="";queries_redirect(remove_from_uri("db|database"),'Database has been dropped.',drop_databases(array(DB)));}elseif(DB!==$B){if(DB!=""){$_GET["db"]=$B;queries_redirect(preg_replace('~\bdb=[^&]*&~','',ME)."db=".urlencode($B),'Database has been renamed.',rename_database($B,$I["collation"]));}else{$i=explode("\n",str_replace("\r","",$B));$Hh=true;$pe="";foreach($i
as$j){if(count($i)==1||$j!=""){if(!create_database($j,$I["collation"]))$Hh=false;$pe=$j;}}restart_session();set_session("dbs",null);queries_redirect(ME."db=".urlencode($pe),'Database has been created.',$Hh);}}else{if(!$I["collation"])redirect(substr(ME,0,-1));query_redirect("ALTER DATABASE ".idf_escape($B).(preg_match('~^[a-z0-9_]+$~i',$I["collation"])?" COLLATE $I[collation]":""),substr(ME,0,-1),'Database has been altered.');}}page_header(DB!=""?'Alter database':'Create database',$l,array(),h(DB));$pb=collations();$B=DB;if($_POST)$B=$I["name"];elseif(DB!="")$I["collation"]=db_collation(DB,$pb);elseif($w=="sql"){foreach(get_vals("SHOW GRANTS")as$td){if(preg_match('~ ON (`(([^\\\\`]|``|\\\\.)*)%`\.\*)?~',$td,$A)&&$A[1]){$B=stripcslashes(idf_unescape("`$A[2]`"));break;}}}echo'
<form action="" method="post">
<p>
',($_POST["add_x"]||strpos($B,"\n")?'<textarea autofocus name="name" rows="10" cols="40">'.h($B).'</textarea><br>':'<input name="name" autofocus value="'.h($B).'" data-maxlength="64" autocapitalize="off">')."\n".($pb?html_select("collation",array(""=>"(".'collation'.")")+$pb,$I["collation"]).doc_link(array('sql'=>"charset-charsets.html",'mariadb'=>"supported-character-sets-and-collations/",'mssql'=>"relational-databases/system-functions/sys-fn-helpcollations-transact-sql",)):""),'<input type="submit" value="Save">
';if(DB!="")echo"<input type='submit' name='drop' value='".'Drop'."'>".confirm(sprintf('Drop %s?',DB))."\n";elseif(!$_POST["add_x"]&&$_GET["db"]=="")echo"<input type='image' class='icon' name='add' src='".h(preg_replace("~\\?.*~","",ME)."?file=plus.gif&version=4.17.1")."' alt='+' title='".'Add next'."'>\n";echo'<input type="hidden" name="token" value="',$S,'">
</form>
';}elseif(isset($_GET["scheme"])){$I=$_POST;if($_POST&&!$l){$z=preg_replace('~ns=[^&]*&~','',ME)."ns=";if($_POST["drop"])query_redirect("DROP SCHEMA ".idf_escape($_GET["ns"]),$z,'Schema has been dropped.');else{$B=trim($I["name"]);$z.=urlencode($B);if($_GET["ns"]=="")query_redirect("CREATE SCHEMA ".idf_escape($B),$z,'Schema has been created.');elseif($_GET["ns"]!=$B)query_redirect("ALTER SCHEMA ".idf_escape($_GET["ns"])." RENAME TO ".idf_escape($B),$z,'Schema has been altered.');else
redirect($z);}}page_header($_GET["ns"]!=""?'Alter schema':'Create schema',$l);if(!$I)$I["name"]=$_GET["ns"];echo'
<form action="" method="post">
<p><input name="name" autofocus value="',h($I["name"]),'" autocapitalize="off">
<input type="submit" value="Save">
';if($_GET["ns"]!="")echo"<input type='submit' name='drop' value='".'Drop'."'>".confirm(sprintf('Drop %s?',$_GET["ns"]))."\n";echo'<input type="hidden" name="token" value="',$S,'">
</form>
';}elseif(isset($_GET["call"])){$da=($_GET["name"]?$_GET["name"]:$_GET["call"]);page_header('Call'.": ".h($da),$l);$Qg=routine($_GET["call"],(isset($_GET["callf"])?"FUNCTION":"PROCEDURE"));$Nd=array();$Jf=array();foreach($Qg["fields"]as$s=>$m){if(substr($m["inout"],-3)=="OUT")$Jf[$s]="@".idf_escape($m["field"])." AS ".idf_escape($m["field"]);if(!$m["inout"]||substr($m["inout"],0,2)=="IN")$Nd[]=$s;}if(!$l&&$_POST){$Za=array();foreach($Qg["fields"]as$x=>$m){if(in_array($x,$Nd)){$X=process_input($m);if($X===false)$X="''";if(isset($Jf[$x]))$f->query("SET @".idf_escape($m["field"])." = $X");}$Za[]=(isset($Jf[$x])?"@".idf_escape($m["field"]):$X);}$F=(isset($_GET["callf"])?"SELECT":"CALL")." ".table($da)."(".implode(", ",$Za).")";$Ch=microtime(true);$G=$f->multi_query($F);$za=$f->affected_rows;echo$b->selectQuery($F,$Ch,!$G);if(!$G)echo"<p class='error'>".error()."\n";else{$g=connect();if(is_object($g))$g->select_db(DB);do{$G=$f->store_result();if(is_object($G))select($G,$g);else
...
';}elseif(isset($_GET["foreign"])){$a=$_GET["foreign"];$B=$_GET["name"];$I=$_POST;if($_POST&&!$l&&!$_POST["add"]&&!$_POST["change"]&&!$_POST["change-js"]){$Me=($_POST["drop"]?'Foreign key has been dropped.':($B!=""?'Foreign key has been altered.':'Foreign key has been created.'));$_=ME."table=".urlencode($a);if(!$_POST["drop"]){$I["source"]=array_filter($I["source"],'strlen');ksort($I["source"]);$Vh=array();foreach($I["source"]as$x=>$X)$Vh[$x]=$I["target"][$x];$I["target"]=$Vh;}if($w=="sqlite")queries_redirect($_,$Me,recreate_table($a,$a,array(),array(),array(" $B"=>($_POST["drop"]?"":" ".format_foreign_key($I)))));else{$c="ALTER TABLE ".table($a);$oc="\nDROP ".($w=="sql"?"FOREIGN KEY ":"CONSTRAINT ").idf_escape($B);if($_POST["drop"])query_redirect($c.$oc,$_,$Me);else{query_redirect($c.($B!=""?"$oc,":"")."\nADD".format_foreign_key($I),$_,$Me);$l='Source and target columns must have the same data type, there must be an index on the target columns and referenced data must exist.'."<br>$l";}}}page_header('Foreign key',$l,array("table"=>$a),h($a));if($_POST){ksort($I["source"]);if($_POST["add"])$I["source"][]="";elseif($_POST["change"]||$_POST["change-js"])$I["target"]=array();}elseif($B!=""){$md=foreign_keys($a);$I=$md[$B];$I["source"][]="";}else{$I["table"]=$a;$I["source"]=array("");}echo'
<form action="" method="post">
';$uh=array_keys(fields($a));if($I["db"]!="")$f->select_db($I["db"]);if($I["ns"]!="")set_schema($I["ns"]);$_g=array_keys(array_filter(table_status('',true),'fk_support'));$Vh=array_keys(fields(in_array($I["table"],$_g)?$I["table"]:reset($_g)));$sf="this.form['change-js'].value = '1'; this.form.submit();";echo"<p>".'Target table'.": ".html_select("table",$_g,$I["table"],$sf)."\n";if($w=="pgsql")echo'Schema'.": ".html_select("ns",$b->schemas(),$I["ns"]!=""?$I["ns"]:$_GET["ns"],$sf);elseif($w!="sqlite"){$Yb=array();foreach($b->databases()as$j){if(!information_schema($j))$Yb[]=$j;}echo'DB'.": ".html_select("db",$Yb,$I["db"]!=""?$I["db"]:$_GET["db"],$sf);}echo'<input type="hidden" name="change-js" value="">
...
';if($B!=""){echo'<input type="submit" name="drop" value="Drop">',confirm(sprintf('Drop %s?',$B));}echo'<input type="hidden" name="token" value="',$S,'">
</form>
';}elseif(isset($_GET["view"])){$a=$_GET["view"];$I=$_POST;$Gf="VIEW";if($w=="pgsql"&&$a!=""){$N=table_status($a);$Gf=strtoupper($N["Engine"]);}if($_POST&&!$l){$B=trim($I["name"]);$Fa=" AS\n$I[select]";$_=ME."table=".urlencode($B);$Me='View has been altered.';$T=($_POST["materialized"]?"MATERIALIZED VIEW":"VIEW");if(!$_POST["drop"]&&$a==$B&&$w!="sqlite"&&$T=="VIEW"&&$Gf=="VIEW")query_redirect(($w=="mssql"?"ALTER":"CREATE OR REPLACE")." VIEW ".table($B).$Fa,$_,$Me);else{$Xh=$B."_adminer_".uniqid();drop_create("DROP $Gf ".table($a),"CREATE $T ".table($B).$Fa,"DROP $T ".table($B),"CREATE $T ".table($Xh).$Fa,"DROP $T ".table($Xh),($_POST["drop"]?substr(ME,0,-1):$_),'View has been dropped.',$Me,'View has been created.',$a,$B);}}if(!$_POST&&$a!=""){$I=view($a);$I["name"]=$a;$I["materialized"]=($Gf!="VIEW");if(!$l)$l=error();}page_header(($a!=""?'Alter view':'Create view'),$l,array("table"=>$a),h($a));echo'
<form action="" method="post">
...
<tr><th>Every<td><input type="number" name="INTERVAL_VALUE" value="',h($I["INTERVAL_VALUE"]),'" class="size"> ',html_select("INTERVAL_FIELD",$Yd,$I["INTERVAL_FIELD"]),'<tr><th>Status<td>',html_select("STATUS",$Dh,$I["STATUS"]),'<tr><th>Comment<td><input name="EVENT_COMMENT" value="',h($I["EVENT_COMMENT"]),'" data-maxlength="64">
...
<input type="submit" value="Save">
';if($aa!=""){echo'<input type="submit" name="drop" value="Drop">',confirm(sprintf('Drop %s?',$aa));}echo'<input type="hidden" name="token" value="',$S,'">
</form>
';}elseif(isset($_GET["procedure"])){$da=($_GET["name"]?$_GET["name"]:$_GET["procedure"]);$Qg=(isset($_GET["function"])?"FUNCTION":"PROCEDURE");$I=$_POST;$I["fields"]=(array)$I["fields"];if($_POST&&!process_fields($I["fields"])&&!$l){$Df=routine($_GET["procedure"],$Qg);$Xh="$I[name]_adminer_".uniqid();drop_create("DROP $Qg ".routine_id($da,$Df),create_routine($Qg,$I),"DROP $Qg ".routine_id($I["name"],$I),create_routine($Qg,array("name"=>$Xh)+$I),"DROP $Qg ".routine_id($Xh,$I),substr(ME,0,-1),'Routine has been dropped.','Routine has been altered.','Routine has been created.',$da,$I["name"]);}page_header(($da!=""?(isset($_GET["function"])?'Alter function':'Alter procedure').": ".h($da):(isset($_GET["function"])?'Create function':'Create procedure')),$l);if(!$_POST&&$da!=""){$I=routine($_GET["procedure"],$Qg);$I["name"]=$da;}$pb=get_vals("SHOW CHARACTER SET");sort($pb);$Rg=routine_languages();echo'
<form action="" method="post" id="form">
<p>Name: <input name="name" value="',h($I["name"]),'" data-maxlength="64" autocapitalize="off">
',($Rg?'Language'.": ".html_select("language",$Rg,$I["language"])."\n":""),'<input type="submit" value="Save">
<div class="scrollable">
<table class="nowrap">
';edit_fields($I["fields"],$pb,$Qg);if(isset($_GET["function"])){echo"<tr><td>".'Return type';edit_type("returns",$I["returns"],$pb,array(),($w=="pgsql"?array("void","trigger"):array()));}echo'</table>
',script("editFields();"),'</div>
<p>';textarea("definition",$I["definition"]);echo'<p>
<input type="submit" value="Save">
';if($da!=""){echo'<input type="submit" name="drop" value="Drop">',confirm(sprintf('Drop %s?',$da));}echo'<input type="hidden" name="token" value="',$S,'">
</form>
';}elseif(isset($_GET["sequence"])){$fa=$_GET["sequence"];$I=$_POST;if($_POST&&!$l){$z=substr(ME,0,-1);$B=trim($I["name"]);if($_POST["drop"])query_redirect("DROP SEQUENCE ".idf_escape($fa),$z,'Sequence has been dropped.');elseif($fa=="")query_redirect("CREATE SEQUENCE ".idf_escape($B),$z,'Sequence has been created.');elseif($fa!=$B)query_redirect("ALTER SEQUENCE ".idf_escape($fa)." RENAME TO ".idf_escape($B),$z,'Sequence has been altered.');else
redirect($z);}page_header($fa!=""?'Alter sequence'.": ".h($fa):'Create sequence',$l);if(!$I)$I["name"]=$fa;echo'
<form action="" method="post">
<p><input name="name" value="',h($I["name"]),'" autocapitalize="off">
<input type="submit" value="Save">
';if($fa!="")echo"<input type='submit' name='drop' value='".'Drop'."'>".confirm(sprintf('Drop %s?',$fa))."\n";echo'<input type="hidden" name="token" value="',$S,'">
</form>
';}elseif(isset($_GET["type"])){$ga=$_GET["type"];$I=$_POST;if($_POST&&!$l){$z=substr(ME,0,-1);if($_POST["drop"])query_redirect("DROP TYPE ".idf_escape($ga),$z,'Type has been dropped.');else
query_redirect("CREATE TYPE ".idf_escape(trim($I["name"]))." $I[as]",$z,'Type has been created.');}page_header($ga!=""?'Alter type'.": ".h($ga):'Create type',$l);if(!$I)$I["as"]="AS ";echo'
<form action="" method="post">
<p>
...
<form action="" method="post">
<p>Name: <input name="name" value="',h($I["name"]),'" data-maxlength="64" autocapitalize="off">',doc_link(array('sql'=>"create-table-check-constraints.html",'mariadb'=>"constraint/",'pgsql'=>"ddl-constraints.html#DDL-CONSTRAINTS-CHECK-CONSTRAINTS",'mssql'=>"relational-databases/tables/create-check-constraints",)),'<p>';textarea("clause",$I["clause"]);echo'<p><input type="submit" value="Save">
';if($B!=""){echo'<input type="submit" name="drop" value="Drop">',confirm(sprintf('Drop %s?',$B));}echo'<input type="hidden" name="token" value="',$S,'">
</form>
';}elseif(isset($_GET["trigger"])){$a=$_GET["trigger"];$B=$_GET["name"];$wi=trigger_options();$I=(array)trigger($B,$a)+array("Trigger"=>$a."_bi");if($_POST){if(!$l&&in_array($_POST["Timing"],$wi["Timing"])&&in_array($_POST["Event"],$wi["Event"])&&in_array($_POST["Type"],$wi["Type"])){$qf=" ON ".table($a);$oc="DROP TRIGGER ".idf_escape($B).($w=="pgsql"?$qf:"");$_=ME."table=".urlencode($a);if($_POST["drop"])query_redirect($oc,$_,'Trigger has been dropped.');else{if($B!="")queries($oc);queries_redirect($_,($B!=""?'Trigger has been altered.':'Trigger has been created.'),queries(create_trigger($qf,$_POST)));if($B!="")queries(create_trigger($qf,$I+array("Type"=>reset($wi["Type"]))));}}$I=$_POST;}page_header(($B!=""?'Alter trigger'.": ".h($B):'Create trigger'),$l,array("table"=>$a));echo'
<form action="" method="post" id="form">
<table class="layout">
<tr><th>Time<td>',html_select("Timing",$wi["Timing"],$I["Timing"],"triggerChange(/^".preg_quote($a,"/")."_[ba][iud]$/, '".js_escape($a)."', this.form);"),'<tr><th>Event<td>',html_select("Event",$wi["Event"],$I["Event"],"this.form['Timing'].onchange();"),(in_array("UPDATE OF",$wi["Event"])?" <input name='Of' value='".h($I["Of"])."' class='hidden'>":""),'<tr><th>Type<td>',html_select("Type",$wi["Type"],$I["Type"]),'</table>
...
as$hf=>$td){echo'<th>'.($hf!="*.*"?"<input name='objects[$s]' value='".h($hf)."' size='10' autocapitalize='off'>":"<input type='hidden' name='objects[$s]' value='*.*' size='10'>*.*");$s++;}echo"</thead>\n";foreach(array(""=>"","Server Admin"=>'Server',"Databases"=>'Database',"Tables"=>'Table',"Columns"=>'Column',"Procedures"=>'Routine',)as$Gb=>$fc){foreach((array)$og[$Gb]as$ng=>$vb){echo"<tr><td".($fc?">$fc<td":" colspan='2'").' lang="en" title="'.h($vb).'">'.h($ng);$s=0;foreach($ud
...
<input type="submit" value="Save">
';if(isset($_GET["host"])){echo'<input type="submit" name="drop" value="Drop">',confirm(sprintf('Drop %s?',"$ha@$_GET[host]"));}echo'<input type="hidden" name="token" value="',$S,'">
</form>
';}elseif(isset($_GET["processlist"])){if(support("kill")){if($_POST&&!$l){$le=0;foreach((array)$_POST["kill"]as$X){if(kill_process($X))$le++;}queries_redirect(ME."processlist=",lang(array('%d process has been killed.','%d processes have been killed.'),$le),$le||!$_POST["kill"]);}}page_header('Process list',$l);echo'
<form action="" method="post">
<div class="scrollable">
...
as$x=>$X)echo"<td>".(($w=="sql"&&$x=="Info"&&preg_match("~Query|Killed~",$I["Command"])&&$X!="")||($w=="pgsql"&&$x=="current_query"&&$X!="<IDLE>")||($w=="oracle"&&$x=="sql_text"&&$X!="")?"<code class='jush-$w'>".shorten_utf8($X,100,"</code>").' <a href="'.h(ME.($I["db"]!=""?"db=".urlencode($I["db"])."&":"")."sql=".urlencode($X)).'">'.'Clone'.'</a>':h($X));echo"\n";}echo'</table>
</div>
<p>
...
as$x=>$X){if(in_array(idf_escape($x),$K))unset($Fi[$x]);}break;}}if($kf&&!$jg){$jg=$Fi=array($kf=>0);$v[]=array("type"=>"PRIMARY","columns"=>array($kf));}if($_POST&&!$l){$gj=$Z;if(!$_POST["all"]&&is_array($_POST["check"])){$gb=array();foreach($_POST["check"]as$cb)$gb[]=where_check($cb,$n);$gj[]="((".implode(") OR (",$gb)."))";}$gj=($gj?"\nWHERE ".implode(" AND ",$gj):"");if($_POST["export"]){cookie("adminer_import","output=".urlencode($_POST["output"])."&format=".urlencode($_POST["format"]));dump_headers($a);$b->dumpTable($a,"");$qd=($K?implode(", ",$K):"*").convert_fields($e,$n,$K)."\nFROM ".table($a);$xd=($vd&&$ce?"\nGROUP BY ".implode(", ",$vd):"").($_f?"\nORDER BY ".implode(", ",$_f):"");if(!is_array($_POST["check"])||$jg)$F="SELECT $qd$gj$xd";else{$Bi=array();foreach($_POST["check"]as$X)$Bi[]="(SELECT".limit($qd,"\nWHERE ".($Z?implode(" AND ",$Z)." AND ":"").where_check($X,$n).$xd,1).")";$F=implode(" UNION ALL ",$Bi);}$b->dumpData($a,"table",$F);exit;}if(!$b->selectEmailProcess($Z,$md)){if($_POST["save"]||$_POST["delete"]){$G=true;$za=0;$M=array();if(!$_POST["delete"]){foreach($e
as$B=>$X){$X=process_input($n[$B]);if($X!==null&&($_POST["clone"]||$X!==false))$M[idf_escape($B)]=($X!==false?$X:idf_escape($B));}}if($_POST["delete"]||$M){if($_POST["clone"])$F="INTO ".table($a)." (".implode(", ",array_keys($M)).")\nSELECT ".implode(", ",$M)."\nFROM ".table($a);if($_POST["all"]||($jg&&is_array($_POST["check"]))||$ce){$G=($_POST["delete"]?$k->delete($a,$gj):($_POST["clone"]?queries("INSERT $F$gj"):$k->update($a,$M,$gj)));$za=$f->affected_rows;}else{foreach((array)$_POST["check"]as$X){$cj="\nWHERE ".($Z?implode(" AND ",$Z)." AND ":"").where_check($X,$n);$G=($_POST["delete"]?$k->delete($a,$cj,1):($_POST["clone"]?queries("INSERT".limit1($a,$F,$cj)):$k->update($a,$M,$cj,1)));if(!$G)break;$za+=$f->affected_rows;}}}$Me=lang(array('%d item has been affected.','%d items have been affected.'),$za);if($_POST["clone"]&&$G&&$za==1){$qe=last_id();if($qe)$Me=sprintf('Item%s has been inserted.'," $qe");}queries_redirect(remove_from_uri($_POST["all"]&&$_POST["delete"]?"page":""),$Me,$G);if(!$_POST["delete"]){edit_form($a,$n,(array)$_POST["fields"],!$_POST["clone"]);page_footer();exit;}}elseif(!$_POST["import"]){if(!$_POST["val"])$l='Ctrl+click on a value to modify it.';else{$G=true;$za=0;foreach($_POST["val"]as$Di=>$I){$M=array();foreach($I
as$x=>$X){$x=bracket_escape($x,1);$M[idf_escape($x)]=(preg_match('~char|text~',$n[$x]["type"])||$X!=""?$b->processInput($n[$x],$X):"NULL");}$G=$k->update($a,$M," WHERE ".($Z?implode(" AND ",$Z)." AND ":"").where_check($Di,$n),!$ce&&!$jg," ");if(!$G)break;$za+=$f->affected_rows;}queries_redirect(remove_from_uri(),lang(array('%d item has been affected.','%d items have been affected.'),$za),$G);}}elseif(!is_string($bd=get_file("csv_file",true)))$l=upload_error($bd);elseif(!preg_match('~~u',$bd))$l='File must be in UTF-8 encoding.';else{cookie("adminer_import","output=".urlencode($ya["output"])."&format=".urlencode($_POST["separator"]));$G=true;$rb=array_keys($n);preg_match_all('~(?>"[^"]*"|[^"\r\n]+)+~',$bd,$De);$za=count($De[0]);$k->begin();$eh=($_POST["separator"]=="csv"?",":($_POST["separator"]=="tsv"?"\t":";"));$J=array();foreach($De[0]as$x=>$X){preg_match_all("~((?>\"[^\"]*\")+|[^$eh]*)$eh~",$X.$eh,$Ee);if(!$x&&!array_diff($Ee[1],$rb)){$rb=$Ee[1];$za--;}else{$M=array();foreach($Ee[1]as$s=>$mb)$M[idf_escape($rb[$s])]=($mb==""&&$n[$rb[$s]]["null"]?"NULL":q(str_replace('""','"',preg_replace('~^"|"$~','',$mb))));$J[]=$M;}}$G=(!$J||$k->insertUpdate($a,$J,$jg));if($G)$G=$k->commit();queries_redirect(remove_from_uri("page"),lang(array('%d row has been imported.','%d rows have been imported.'),$za),$G);$k->rollback();}}}$Nh=$b->tableName($Q);if(is_ajax()){page_headers();ob_start();}else
page_header('Select'.": $Nh",$l);$M=null;if(isset($Og["insert"])||!support("table")){$Pf=array();foreach((array)$_GET["where"]as$X){if(isset($md[$X["col"]])&&count($md[$X["col"]])==1&&($X["op"]=="="||(!$X["op"]&&(is_array($X["val"])||!preg_match('~[_%]~',$X["val"])))))$Pf["set"."[".bracket_escape($X["col"])."]"]=$X["val"];}$M=$Pf?"&".http_build_query($Pf):"";}$b->selectLinks($Q,$M);if(!$e&&support("table"))echo"<p class='error'>".'Unable to select the table'.($n?".":": ".error())."\n";else{echo"<form action='' id='form'>\n","<div style='display: none;'>";hidden_fields_get();echo(DB!=""?'<input type="hidden" name="db" value="'.h(DB).'">'.(isset($_GET["ns"])?'<input type="hidden" name="ns" value="'.h($_GET["ns"]).'">':""):"");echo'<input type="hidden" name="select" value="'.h($a).'">',"</div>\n";$b->selectColumnsPrint($K,$e);$b->selectSearchPrint($Z,$e,$v);$b->selectOrderPrint($_f,$e,$v);$b->selectLimitPrint($y);$b->selectLengthPrint($bi);$b->selectActionPrint($v);echo"</form>\n";$D=$_GET["page"];if($D=="last"){$pd=$f->result(count_rows($a,$Z,$ce,$vd));$D=floor(max(0,$pd-1)/$y);}$Zg=$K;$wd=$vd;if(!$Zg){$Zg[]="*";$Hb=convert_fields($e,$n,$K);if($Hb)$Zg[]=substr($Hb,2);}foreach($K
as$x=>$X){$m=$n[idf_unescape($X)];if($m&&($Fa=convert_field($m)))$Zg[$x]="$Fa AS $X";}if(!$ce&&$Fi){foreach($Fi
as$x=>$X){$Zg[]=idf_escape($x);if($wd)$wd[]=idf_escape($x);}}$G=$k->select($a,$Zg,$Z,$wd,$_f,$y,$D,true);if(!$G)echo"<p class='error'>".error()."\n";else{if($w=="mssql"&&$D)$G->seek($y*$D);$Ac=array();echo"<form action='' method='post' enctype='multipart/form-data'>\n";$J=array();while($I=$G->fetch_assoc()){if($D&&$w=="oracle")unset($I["RNUM"]);$J[]=$I;}if($_GET["page"]!="last"&&$y!=""&&$vd&&$ce&&$w=="sql")$pd=$f->result(" SELECT FOUND_ROWS()");if(!$J)echo"<p class='message'>".'No rows.'."\n";else{$Oa=$b->backwardKeys($a,$Nh);echo"<div class='scrollable'>","<table id='table' class='nowrap checkable odds'>",script("mixin(qs('#table'), {onclick: tableClick, ondblclick: partialArg(tableClick, true), onkeydown: editingKeydown});"),"<thead><tr>".(!$vd&&$K?"":"<td><input type='checkbox' id='all-page' class='jsonly'>".script("qs('#all-page').onclick = partial(formCheck, /check/);","")." <a href='".h($_GET["modify"]?remove_from_uri("modify"):$_SERVER["REQUEST_URI"]."&modify=1")."'>".'Modify'."</a>");$Xe=array();$rd=array();reset($K);$xg=1;foreach($J[0]as$x=>$X){if(!isset($Fi[$x])){$X=$_GET["columns"][key($K)];$m=$n[$K?($X?$X["col"]:current($K)):$x];$B=($m?$b->fieldName($m,$xg):($X["fun"]?"*":h($x)));if($B!=""){$xg++;$Xe[$x]=$B;$d=idf_escape($x);$Id=remove_from_uri('(order|desc)[^=]*|page').'&order%5B0%5D='.urlencode($x);$fc="&desc%5B0%5D=1";echo"<th id='th[".h(bracket_escape($x))."]'>".script("mixin(qsl('th'), {onmouseover: partial(columnMouse), onmouseout: partial(columnMouse, ' hidden')});",""),'<a href="'.h($Id.($_f[0]==$d||$_f[0]==$x||(!$_f&&$ce&&$vd[0]==$d)?$fc:'')).'">';echo
apply_sql_function($X["fun"],$B)."</a>";echo"<span class='column hidden'>","<a href='".h($Id.$fc)."' title='".'descending'."' class='text'> â†“</a>";if(!$X["fun"]){echo'<a href="#fieldset-search" title="'.'Search'.'" class="text jsonly"> =</a>',script("qsl('a').onclick = partial(selectSearch, '".js_escape($x)."');");}echo"</span>";}$rd[$x]=$X["fun"];next($K);}}$we=array();if($_GET["modify"]){foreach($J
as$I){foreach($I
as$x=>$X)$we[$x]=max($we[$x],min(40,strlen(utf8_decode($X))));}}echo($Oa?"<th>".'Relations':"")."</thead>\n";if(is_ajax())ob_end_clean();foreach($b->rowDescriptions($J,$md)as$We=>$I){$Ci=unique_array($J[$We],$v);if(!$Ci){$Ci=array();foreach($J[$We]as$x=>$X){if(!preg_match('~^(COUNT\((\*|(DISTINCT )?`(?:[^`]|``)+`)\)|(AVG|GROUP_CONCAT|MAX|MIN|SUM)\(`(?:[^`]|``)+`\))$~',$x))$Ci[$x]=$X;}}$Di="";foreach($Ci
as$x=>$X){if(($w=="sql"||$w=="pgsql")&&preg_match('~char|text|enum|set~',$n[$x]["type"])&&strlen($X)>64){$x=(strpos($x,'(')?$x:idf_escape($x));$x="MD5(".($w!='sql'||preg_match("~^utf8~",$n[$x]["collation"])?$x:"CONVERT($x USING ".charset($f).")").")";$X=md5($X);}$Di.="&".($X!==null?urlencode("where[".bracket_escape($x)."]")."=".urlencode($X===false?"f":$X):"null%5B%5D=".urlencode($x));}echo"<tr>".(!$vd&&$K?"":"<td>".checkbox("check[]",substr($Di,1),in_array(substr($Di,1),(array)$_POST["check"])).($ce||information_schema(DB)?"":" <a href='".h(ME."edit=".urlencode($a).$Di)."' class='edit'>".'edit'."</a>"));foreach($I
...
as$he=>$W)$z.=where_link($s++,$he,$W);}$X=select_value($X,$z,$m,$bi);$Jd=h("val[$Di][".bracket_escape($x)."]");$Y=$_POST["val"][$Di][bracket_escape($x)];$wc=!is_array($I[$x])&&is_utf8($X)&&$J[$We][$x]==$I[$x]&&!$rd[$x];$Zh=preg_match('~text|lob~',$m["type"]);echo"<td id='$Jd'";if(($_GET["modify"]&&$wc)||$Y!==null){$_d=h($Y!==null?$Y:$I[$x]);echo">".($Zh?"<textarea name='$Jd' cols='30' rows='".(substr_count($I[$x],"\n")+1)."'>$_d</textarea>":"<input name='$Jd' value='$_d' size='$we[$x]'>");}else{$_e=strpos($X,"<i>â€¦</i>");echo" data-text='".($_e?2:($Zh?1:0))."'".($wc?"":" data-warning='".h('Use edit link to modify this value.')."'").">$X</td>";}}}if($Oa)echo"<td>";$b->backwardKeysPrint($Oa,$J[$We]);echo"</tr>\n";}if(is_ajax())exit;echo"</table>\n","</div>\n";}if(!is_ajax()){if($J||$D){$Mc=true;if($_GET["page"]!="last"){if($y==""||(count($J)<$y&&($J||!$D)))$pd=($D?$D*$y:0)+count($J);elseif($w!="sql"||!$ce){$pd=($ce?false:found_rows($Q,$Z));if($pd<max(1e4,2*($D+1)*$y))$pd=reset(slow_query(count_rows($a,$Z,$ce,$vd)));else$Mc=false;}}$Nf=($y!=""&&($pd===false||$pd>$y||$D));if($Nf){echo(($pd===false?count($J)+1:$pd-$D*$y)>$y?'<p><a href="'.h(remove_from_uri("page")."&page=".($D+1)).'" class="loadmore">'.'Load more data'.'</a>'.script("qsl('a').onclick = partial(selectLoadMore, ".(+$y).", '".'Loading'."â€¦');",""):''),"\n";}}echo"<div class='footer'><div>\n";if($J||$D){if($Nf){$Ge=($pd===false?$D+(count($J)>=$y?2:1):floor(($pd-1)/$y));echo"<fieldset>";if($w!="simpledb"){echo"<legend><a href='".h(remove_from_uri("page"))."'>".'Page'."</a></legend>",script("qsl('a').onclick = function () { pageClick(this.href, +prompt('".'Page'."', '".($D+1)."')); return false; };"),pagination(0,$D).($D>5?" â€¦":"");for($s=max(1,$D-4);$s<min($Ge,$D+5);$s++)echo
pagination($s,$D);if($Ge>0){echo($D+5<$Ge?" â€¦":""),($Mc&&$pd!==false?pagination($Ge,$D):" <a href='".h(remove_from_uri("page")."&page=last")."' title='~$Ge'>".'last'."</a>");}}else{echo"<legend>".'Page'."</legend>",pagination(0,$D).($D>1?" â€¦":""),($D?pagination($D,$D):""),($Ge>$D?pagination($D+1,$D).($Ge>$D+1?" â€¦":""):"");}echo"</fieldset>\n";}echo"<fieldset>","<legend>".'Whole result'."</legend>";$lc=($Mc?"":"~ ").$pd;echo
checkbox("all",1,0,($pd!==false?($Mc?"":"~ ").lang(array('%d row','%d rows'),$pd):""),"var checked = formChecked(this, /check/); selectCount('selected', this.checked ? '$lc' : checked); selectCount('selected2', this.checked || !checked ? '$lc' : checked);")."\n","</fieldset>\n";if($b->selectCommandPrint()){echo'<fieldset',($_GET["modify"]?'':' class="jsonly"'),'><legend>Modify</legend><div>
...
';}$nd=$b->dumpFormat();foreach((array)$_GET["columns"]as$d){if($d["fun"]){unset($nd['sql']);break;}}if($nd){print_fieldset("export",'Export'." <span id='selected2'></span>");$Kf=$b->dumpOutput();echo($Kf?html_select("output",$Kf,$ya["output"])." ":""),html_select("format",$nd,$ya["format"])," <input type='submit' name='export' value='".'Export'."'>\n","</div></fieldset>\n";}$b->selectEmailPrint(array_filter($Ac,'strlen'),$e);}echo"</div></div>\n";if($b->selectImportPrint()){echo"<div>","<a href='#import'>".'Import'."</a>",script("qsl('a').onclick = partial(toggle, 'import');",""),"<span id='import' class='hidden'>: ","<input type='file' name='csv_file'> ",html_select("separator",array("csv"=>"CSV,","csv;"=>"CSV;","tsv"=>"TSV"),$ya["format"],1);echo" <input type='submit' name='import' value='".'Import'."'>","</span>","</div>";}echo"<input type='hidden' name='token' value='$S'>\n","</form>\n",(!$vd&&$K?"":script("tableCheck();"));}}}if(is_ajax()){ob_end_clean();exit;}}elseif(isset($_GET["variables"])){$N=isset($_GET["status"]);page_header($N?'Status':'Variables');$Ti=($N?show_status():show_variables());if(!$Ti)echo"<p class='message'>".'No rows.'."\n";else{echo"<table>\n";foreach($Ti
...
as$x=>$X)json_row("sum-$x",format_number($X));json_row("");}elseif($_GET["script"]=="kill")$f->query("KILL ".number($_POST["kill"]));else{foreach(count_tables($b->databases())as$j=>$X){json_row("tables-$j",$X);json_row("size-$j",db_size($j));}json_row("");}exit;}else{$Th=array_merge((array)$_POST["tables"],(array)$_POST["views"]);if($Th&&!$l&&!$_POST["search"]){$G=true;$Me="";if($w=="sql"&&$_POST["tables"]&&count($_POST["tables"])>1&&($_POST["drop"]||$_POST["truncate"]||$_POST["copy"]))queries("SET foreign_key_checks = 0");if($_POST["truncate"]){if($_POST["tables"])$G=truncate_tables($_POST["tables"]);$Me='Tables have been truncated.';}elseif($_POST["move"]){$G=move_tables((array)$_POST["tables"],(array)$_POST["views"],$_POST["target"]);$Me='Tables have been moved.';}elseif($_POST["copy"]){$G=copy_tables((array)$_POST["tables"],(array)$_POST["views"],$_POST["target"]);$Me='Tables have been copied.';}elseif($_POST["drop"]){if($_POST["views"])$G=drop_views($_POST["views"]);if($G&&$_POST["tables"])$G=drop_tables($_POST["tables"]);$Me='Tables have been dropped.';}elseif($w!="sql"){$G=($w=="sqlite"?queries("VACUUM"):apply_queries("VACUUM".($_POST["optimize"]?"":" ANALYZE"),$_POST["tables"]));$Me='Tables have been optimized.';}elseif(!$_POST["tables"])$Me='No tables.';elseif($G=queries(($_POST["optimize"]?"OPTIMIZE":($_POST["check"]?"CHECK":($_POST["repair"]?"REPAIR":"ANALYZE")))." TABLE ".implode(", ",array_map('idf_escape',$_POST["tables"])))){while($I=$G->fetch_assoc())$Me.="<b>".h($I["Table"])."</b>: ".h($I["Msg_text"])."<br>";}queries_redirect(substr(ME,0,-1),$Me,$G);}page_header(($_GET["ns"]==""?'Database'.": ".h(DB):'Schema'.": ".h($_GET["ns"])),$l,true);if($b->homepage()){if($_GET["ns"]!==""){echo"<h3 id='tables-views'>".'Tables and views'."</h3>\n";$Sh=tables_list();if(!$Sh)echo"<p class='message'>".'No tables.'."\n";else{echo"<form action='' method='post'>\n";if(support("table")){echo"<fieldset><legend>".'Search data in tables'." <span id='selected2'></span></legend><div>","<input type='search' name='query' value='".h($_POST["query"])."'>",script("qsl('input').onkeydown = partialArg(bodyKeydown, 'search');","")," <input type='submit' name='search' value='".'Search'."'>\n","</div></fieldset>\n";if($_POST["search"]&&$_POST["query"]!=""){$_GET["where"][0]["op"]=$k->convertOperator("LIKE %%");search_tables();}}echo"<div class='scrollable'>\n","<table class='nowrap checkable odds'>\n",script("mixin(qsl('table'), {onclick: tableClick, ondblclick: partialArg(tableClick, true)});"),'<thead><tr class="wrap">','<td><input id="check-all" type="checkbox" class="jsonly">'.script("qs('#check-all').onclick = partial(formCheck, /^(tables|views)\[/);",""),'<th>'.'Table','<td>'.'Engine'.doc_link(array('sql'=>'storage-engines.html')),'<td>'.'Collation'.doc_link(array('sql'=>'charset-charsets.html','mariadb'=>'supported-character-sets-and-collations/')),'<td>'.'Data Length'.doc_link(array('sql'=>'show-table-status.html','pgsql'=>'functions-admin.html#FUNCTIONS-ADMIN-DBOBJECT','oracle'=>'REFRN20286')),'<td>'.'Index Length'.doc_link(array('sql'=>'show-table-status.html','pgsql'=>'functions-admin.html#FUNCTIONS-ADMIN-DBOBJECT')),'<td>'.'Data Free'.doc_link(array('sql'=>'show-table-status.html')),'<td>'.'Auto Increment'.doc_link(array('sql'=>'example-auto-increment.html','mariadb'=>'auto_increment/')),'<td>'.'Rows'.doc_link(array('sql'=>'show-table-status.html','pgsql'=>'catalog-pg-class.html#CATALOG-PG-CLASS','oracle'=>'REFRN20286')),(support("comment")?'<td>'.'Comment'.doc_link(array('sql'=>'show-table-status.html','pgsql'=>'functions-info.html#FUNCTIONS-INFO-COMMENT-TABLE')):''),"</thead>\n";$R=0;foreach($Sh
as$B=>$T){$Wi=($T!==null&&!preg_match('~table|sequence~i',$T));$Jd=h("Table-".$B);echo'<tr><td>'.checkbox(($Wi?"views[]":"tables[]"),$B,in_array($B,$Th,true),"","","",$Jd),'<th>'.(support("table")||support("indexes")?"<a href='".h(ME)."table=".urlencode($B)."' title='".'Show structure'."' id='$Jd'>".h($B).'</a>':h($B));if($Wi){echo'<td colspan="6"><a href="'.h(ME)."view=".urlencode($B).'" title="'.'Alter view'.'">'.(preg_match('~materialized~i',$T)?'Materialized view':'View').'</a>','<td align="right"><a href="'.h(ME)."select=".urlencode($B).'" title="'.'Select data'.'">?</a>';}else{foreach(array("Engine"=>array(),"Collation"=>array(),"Data_length"=>array("create",'Alter table'),"Index_length"=>array("indexes",'Alter indexes'),"Data_free"=>array("edit",'New item'),"Auto_increment"=>array("auto_increment=1&create",'Alter table'),"Rows"=>array("select",'Select data'),)as$x=>$z){$Jd=" id='$x-".h($B)."'";echo($z?"<td align='right'>".(support("table")||$x=="Rows"||(support("indexes")&&$x!="Data_length")?"<a href='".h(ME."$z[0]=").urlencode($B)."'$Jd title='$z[1]'>?</a>":"<span$Jd>?</span>"):"<td id='$x-".h($B)."'>");}$R++;}echo(support("comment")?"<td id='Comment-".h($B)."'>":""),"\n";}echo"<tr><td><th>".sprintf('%d in total',count($Sh)),"<td>".h($w=="sql"?$f->result("SELECT @@default_storage_engine"):""),"<td>".h(db_collation(DB,collations()));foreach(array("Data_length","Index_length","Data_free")as$x)echo"<td align='right' id='sum-$x'>";echo"\n","</table>\n","</div>\n";if(!information_schema(DB)){echo"<div class='footer'><div>\n";$Qi="<input type='submit' value='".'Vacuum'."'> ".on_help("'VACUUM'");$xf="<input type='submit' name='optimize' value='".'Optimize'."'> ".on_help($w=="sql"?"'OPTIMIZE TABLE'":"'VACUUM OPTIMIZE'");echo"<fieldset><legend>".'Selected'." <span id='selected'></span></legend><div>".($w=="sqlite"?$Qi:($w=="pgsql"?$Qi.$xf:($w=="sql"?"<input type='submit' value='".'Analyze'."'> ".on_help("'ANALYZE TABLE'").$xf."<input type='submit' name='check' value='".'Check'."'> ".on_help("'CHECK TABLE'")."<input type='submit' name='repair' value='".'Repair'."'> ".on_help("'REPAIR TABLE'"):"")))."<input type='submit' name='truncate' value='".'Truncate'."'> ".on_help($w=="sqlite"?"'DELETE'":"'TRUNCATE".($w=="pgsql"?"'":" TABLE'")).confirm()."<input type='submit' name='drop' value='".'Drop'."'>".on_help("'DROP TABLE'").confirm()."\n";$i=(support("scheme")?$b->schemas():$b->databases());if(count($i)!=1&&$w!="sqlite"){$j=(isset($_POST["target"])?$_POST["target"]:(support("scheme")?$_GET["ns"]:DB));echo"<p>".'Move to other database'.": ",($i?html_select("target",$i,$j):'<input name="target" value="'.h($j).'" autocapitalize="off">')," <input type='submit' name='move' value='".'Move'."'>",(support("copy")?" <input type='submit' name='copy' value='".'Copy'."'> ".checkbox("overwrite",1,$_POST["overwrite"],'overwrite'):""),"\n";}echo"<input type='hidden' name='all' value=''>";echo
